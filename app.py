...
# ВАЖНО: тут у вас была ошибка — не было запятой между "sdamgia.ru" и "www.virustotal.com"
WHITELISTED_DOMAINS = {
    "example.com",
    "trusted-site.org",
    "safe-site.com",
...
def check_suspicious_words(url: str) -> tuple[bool, str, int]:
    ...
    try:
        ...
        if suspicious_found:
            ...
            return False, f"Обнаружены подозрительные слова: {', '.join(suspicious_found)}", violations
...
def check_virustotal(url: str) -> tuple[bool | None, str, int]:
    """
    Возвращает:
...
@app.get("/check")
def check(url: str = Query(..., min_length=4, description="URL to check, must include https://")):
    if not url:
